...
def sum_rec(
    items: list, cumulative_liters: int, curr_permutation: set, next_container_pos: int
):
    ...
    while (
        next_container_pos < item_len
        and cumulative_liters + items[next_container_pos] <= TARGET_LITTERS
        and next_container_pos not in curr_permutation
    ):
        MCOUNT += 1
        sub_permutations_ = sum_rec(
            items, cumulative_liters, curr_permutation.copy(), next_container_pos + 1
        )
